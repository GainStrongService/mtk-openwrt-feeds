...
#define SEQ_GEN_L(idx)	(NPU_SEQ_GEN_BASE + (idx) * 0x20)
#define SEQ_GEN_H(idx)	(NPU_SEQ_GEN_BASE + (idx) * 0x20 + 0x10)

static DECLARE_BITMAP(seq_gen_used, NPU_SEQ_GEN_IDX_MAX);
...
{
	if (seq_gen_idx >= NPU_SEQ_GEN_IDX_MAX)
		return -EINVAL;
...
static void mtk_npu_seq_gen_set_16_no_lock(int seq_gen_idx, u16 val)
{
...
static void mtk_npu_seq_gen_set_32_no_lock(int seq_gen_idx, u32 val)
{
	if (unlikely(!test_bit(seq_gen_idx, seq_gen_used)))
		return;
...
static int mtk_npu_seq_gen_next_16_no_lock(int seq_gen_idx, u16 *val)
{
...
static int mtk_npu_seq_gen_next_32_no_lock(int seq_gen_idx, u32 *val)
{
	if (unlikely(!val || !test_bit(seq_gen_idx, seq_gen_used)))
		return -EINVAL;
...
void mtk_npu_seq_gen_set_16(int seq_gen_idx, u16 val)
{
	...
	mtk_npu_seq_gen_set_16_no_lock(seq_gen_idx, val);
...
int mtk_npu_seq_gen_next_16(int seq_gen_idx, u16 *val)
{
	...
	ret = mtk_npu_seq_gen_next_16_no_lock(seq_gen_idx, val);
...
void mtk_npu_seq_gen_set_32(int seq_gen_idx, u32 val)
{
	...
	mtk_npu_seq_gen_set_32_no_lock(seq_gen_idx, val);
...
int mtk_npu_seq_gen_next_32(int seq_gen_idx, u32 *val)
{
	...
	ret = mtk_npu_seq_gen_next_32_no_lock(seq_gen_idx, val);
...
static int mtk_npu_seq_gen_alloc_no_lock(int *seq_gen_idx)
{
	if (!seq_gen_idx)
		return -EINVAL;

	*seq_gen_idx = find_first_zero_bit(seq_gen_used, NPU_SEQ_GEN_IDX_MAX);
	if (*seq_gen_idx == NPU_SEQ_GEN_IDX_MAX) {
		NPU_NOTICE("Sequence generator exhausted\n");
		return -ENOMEM;
	}
...
int mtk_npu_seq_gen_alloc(int *seq_gen_idx)
{
	...
	ret = mtk_npu_seq_gen_alloc_no_lock(seq_gen_idx);
...
static void mtk_npu_seq_gen_free_no_lock(int seq_gen_idx)
{
...
void mtk_npu_seq_gen_free(int seq_gen_idx)
{
	...
	mtk_npu_seq_gen_free_no_lock(seq_gen_idx);
