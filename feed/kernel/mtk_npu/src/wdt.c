...
static void wdt_ser_callback(struct npu_ser_params *ser_params)
{
	if (ser_params->type != NPU_SER_WDT_TO)
		WARN_ON(1);

	mtk_trm_dump(ser_params->data.wdt.timeout_cores);
}

static void wdt_ser_mcmd_setup(struct npu_ser_params *ser_params,
			       struct mcu_ctrl_cmd *mcmd)
{
	mcmd->core_mask = (~ser_params->data.wdt.timeout_cores) & CORE_NPU_MASK;
}
...
int mtk_npu_wdt_trigger_timeout(enum core_id core)
{
	...
	if (core == CORE_MGMT)
		return mbox_send_msg_no_wait(&wdt.mgmt_mdev, &msg);
	else
		return mbox_send_msg_no_wait(&wdt.offload_mdev[core], &msg);
}

static int mtk_npu_wdt_register_mbox(void)
{
	int ret;
	int i;
	...
	if (ret) {
		NPU_ERR("register wdt mgmt mbox send failed: %d\n", ret);
		return ret;
	}
	...
	for (i = 0; i < CORE_OFFLOAD_NUM; i++) {
		...
		if (ret) {
			NPU_ERR("register wdt offload %d mbox send failed: %d\n",
				 i, ret);
...
static void mtk_npu_wdt_unregister_mbox(void)
{
	int i;
...
int mtk_npu_wdt_init(struct platform_device *pdev)
{
	...
	ret = devm_request_irq(npu.dev, irq,
			       ...
			       pdev->name, NULL);
	if (ret) {
		NPU_ERR("request wdt irq failed\n");
		return ret;
	}

	ret = mtk_npu_wdt_register_mbox();
	if (ret)
		return ret;
