...
static struct hpdma_dev *chan_to_hpdma_dev(struct dma_chan *chan)
{
	return container_of(chan->device, struct hpdma_dev, ddev);
}

static struct hpdma_vchan *chan_to_hpdma_vchan(struct dma_chan *chan)
{
	return container_of(chan, struct hpdma_vchan, vchan.chan);
}

static struct hpdma_vdesc *vdesc_to_hpdma_vdesc(struct virt_dma_desc *vdesc)
{
					   ...
					   struct hpdma_vdesc *hvdesc)
{
	...
	mtk_npu_hwspin_lock(hpdma->hwspinlock_grp, hpdma->trigger_start_slot);
	...
	/* trigger dma start */
	hpdma_set(hpdma, NPU_HPDMA_X_START(hvchan->pchan_id), HPDMA_START);
...
terminate_transfer:
	...
	mtk_npu_hwspin_unlock(hpdma->hwspinlock_grp, hpdma->trigger_start_slot);
					  ...
					  struct hpdma_vdesc *hvdesc)
{
	/* update axsize */
	hpdma_rmw(hpdma,
		  NPU_HPDMA_X_CTRL(hvchan->pchan_id),
		  HPDMA_AXSIZE_MASK,
		  FIELD_PREP(HPDMA_AXSIZE_MASK, hvdesc->axsize));

	/* update total num */
	hpdma_rmw(hpdma,
		  NPU_HPDMA_X_NUM(hvchan->pchan_id),
		  HPDMA_TOTALNUM_MASK,
		  FIELD_PREP(HPDMA_TOTALNUM_MASK, hvdesc->total_num));

	/* set src addr */
	hpdma_write(hpdma, NPU_HPDMA_X_SRC(hvchan->pchan_id), hvdesc->src);

	/* set dst addr */
	hpdma_write(hpdma, NPU_HPDMA_X_DST(hvchan->pchan_id), hvdesc->dst);
}
...
static int mtk_hpdma_probe(struct platform_device *pdev)
{
	 ...
	 * we need NPU mailbox to notify us when hpdma done
	 */
...
static void mtk_hpdma_remove(struct platform_device *pdev)
					  ...
					  struct hpdma_vdesc *hvdesc)
{
	...
	while (!mtk_npu_hwspin_try_lock(grp, slot)) {
	...
	mtk_npu_hwspin_unlock(grp, slot);
						    ...
						    struct mailbox_msg *msg)
{
	...
	mtk_npu_hwspin_unlock(grp, hpdma->trigger_start_slot);
...
int __init mtk_npu_hpdma_init(void)
{
	int ret = 0;
...
void __exit mtk_npu_hpdma_exit(void)
{
