...
static irqreturn_t mtk_npu_mbox_handler(int irq, void *dev_id)
{
	struct mailbox_reg mreg = {0};
	u32 cluster_reg = 0;
	u32 top_reg = 0;

	top_reg = mbox_read(NPU_TOP_AP_SLOT);
	cluster_reg = mbox_read(NPU_CLUST0_AP_SLOT);

	if (top_reg & MBOX_TOP_MBOX_FROM_CM) {
		mreg.cmd_set_reg = NPU_TOP_CM_TO_AP_CMD_SET;
		mreg.cmd_clr_reg = NPU_TOP_CM_TO_AP_CMD_CLR;
		mreg.msg_reg = NPU_TOP_CM_TO_AP_MSG_N(0);
		exec_mbox_handler(CORE_MGMT, &mreg);
	}
	if (cluster_reg & MBOX_CLUST0_MBOX_FROM_C0) {
		mreg.cmd_set_reg = NPU_CLUST0_CX_TO_AP_CMD_SET(0);
		mreg.cmd_clr_reg = NPU_CLUST0_CX_TO_AP_CMD_CLR(0);
		mreg.msg_reg = NPU_CLUST0_CX_TO_AP_MSG_N(0, 0);
		exec_mbox_handler(CORE_OFFLOAD_0, &mreg);
	}
	if (cluster_reg & MBOX_CLUST0_MBOX_FROM_C1) {
		mreg.cmd_set_reg = NPU_CLUST0_CX_TO_AP_CMD_SET(1);
		mreg.cmd_clr_reg = NPU_CLUST0_CX_TO_AP_CMD_CLR(1);
		mreg.msg_reg = NPU_CLUST0_CX_TO_AP_MSG_N(1, 0);
		exec_mbox_handler(CORE_OFFLOAD_1, &mreg);
	}
	if (cluster_reg & MBOX_CLUST0_MBOX_FROM_C2) {
		mreg.cmd_set_reg = NPU_CLUST0_CX_TO_AP_CMD_SET(2);
		mreg.cmd_clr_reg = NPU_CLUST0_CX_TO_AP_CMD_CLR(2);
		mreg.msg_reg = NPU_CLUST0_CX_TO_AP_MSG_N(2, 0);
		exec_mbox_handler(CORE_OFFLOAD_2, &mreg);
	}
	if (cluster_reg & MBOX_CLUST0_MBOX_FROM_C3) {
		mreg.cmd_set_reg = NPU_CLUST0_CX_TO_AP_CMD_SET(3);
		mreg.cmd_clr_reg = NPU_CLUST0_CX_TO_AP_CMD_CLR(3);
		mreg.msg_reg = NPU_CLUST0_CX_TO_AP_MSG_N(3, 0);
...
{
	if (!mdev) {
		NPU_NOTICE("no mdev specified!\n");
		return -EINVAL;
	}

	if (mdev->core == CORE_MGMT) {
		mbox_reg->cmd_set_reg = NPU_TOP_AP_TO_CM_CMD_SET;
		mbox_reg->cmd_clr_reg = NPU_TOP_AP_TO_CM_CMD_CLR;
		mbox_reg->msg_reg = NPU_TOP_AP_TO_CM_MSG_N(0);
	} else if (mdev->core == CORE_OFFLOAD_0) {
		mbox_reg->cmd_set_reg = NPU_CLUST0_AP_TO_CX_CMD_SET(0);
		mbox_reg->cmd_clr_reg = NPU_CLUST0_AP_TO_CX_CMD_CLR(0);
		mbox_reg->msg_reg = NPU_CLUST0_AP_TO_CX_MSG_N(0, 0);
	} else if (mdev->core == CORE_OFFLOAD_1) {
		mbox_reg->cmd_set_reg = NPU_CLUST0_AP_TO_CX_CMD_SET(1);
		mbox_reg->cmd_clr_reg = NPU_CLUST0_AP_TO_CX_CMD_CLR(1);
		mbox_reg->msg_reg = NPU_CLUST0_AP_TO_CX_MSG_N(1, 0);
	} else if (mdev->core == CORE_OFFLOAD_2) {
		mbox_reg->cmd_set_reg = NPU_CLUST0_AP_TO_CX_CMD_SET(2);
		mbox_reg->cmd_clr_reg = NPU_CLUST0_AP_TO_CX_CMD_CLR(2);
		mbox_reg->msg_reg = NPU_CLUST0_AP_TO_CX_MSG_N(2, 0);
	} else if (mdev->core == CORE_OFFLOAD_3) {
		mbox_reg->cmd_set_reg = NPU_CLUST0_AP_TO_CX_CMD_SET(3);
		mbox_reg->cmd_clr_reg = NPU_CLUST0_AP_TO_CX_CMD_CLR(3);
		mbox_reg->msg_reg = NPU_CLUST0_AP_TO_CX_MSG_N(3, 0);
	} else {
		NPU_NOTICE("invalid mdev->core: %u\n", mdev->core);
		return -EINVAL;
	}
					      ...
					      struct mailbox_reg *mbox_reg)
{
	...
	if (!mdev || !msg || !mbox_reg) {
		NPU_NOTICE("missing some necessary parameters!\n");
		return -EPERM;
	}
	...
	while (mbox_read(mbox_reg->cmd_set_reg)) {
		if (mbox_send_msg_chk_timeout(start)) {
			NPU_NOTICE("mbox occupied too long\n");
			NPU_NOTICE("cmd set reg (0x%x): 0x%x\n",
				   mbox_reg->cmd_set_reg,
				   mbox_read(mbox_reg->cmd_set_reg));
			NPU_NOTICE("msg1 reg (0x%x): 0x%x\n",
				   mbox_reg->msg_reg,
				   mbox_read(mbox_reg->msg_reg));
			NPU_NOTICE("msg2 reg (0x%x): 0x%x\n",
				   mbox_reg->msg_reg,
				   mbox_read(mbox_reg->msg_reg + 0x4));
			NPU_NOTICE("msg3 reg (0x%x): 0x%x\n",
				   mbox_reg->msg_reg,
				   mbox_read(mbox_reg->msg_reg + 0x8));
			NPU_NOTICE("msg4 reg (0x%x): 0x%x\n",
				   mbox_reg->msg_reg,
				   mbox_read(mbox_reg->msg_reg + 0xC));
			NPU_NOTICE("msg5 reg (0x%x): 0x%x\n",
				   mbox_reg->msg_reg,
...
void mtk_npu_mbox_clear_all_cmd(void)
{
	u32 i, j;

	mbox_write(NPU_TOP_AP_TO_CM_CMD_CLR, 0xFFFFFFFF);
	mbox_write(NPU_TOP_CM_TO_AP_CMD_CLR, 0xFFFFFFFF);

	for (i = 0; i < CORE_OFFLOAD_NUM; i++) {
		mbox_write(NPU_CLUST0_CX_TO_CM_CMD_CLR(i), 0xFFFFFFFF);
		mbox_write(NPU_CLUST0_CM_TO_CX_CMD_CLR(i), 0xFFFFFFFF);
		mbox_write(NPU_CLUST0_CX_TO_AP_CMD_CLR(i), 0xFFFFFFFF);
		mbox_write(NPU_CLUST0_AP_TO_CX_CMD_CLR(i), 0xFFFFFFFF);

		for (j = 0; j < CORE_OFFLOAD_NUM; j++) {
			if (i == j)
				continue;

			mbox_write(NPU_CLUST0_CX_TO_CY_CMD_CLR(i, j), 0xFFFFFFFF);
		}
	}
}

int mtk_npu_mbox_init(struct platform_device *pdev)
{
	uint32_t i;
	int ret;
	int irq;
	...
	if (irq < 0) {
		NPU_NOTICE("get mailbox irq handle failed\n");
		return irq;
	}

	ret = devm_request_irq(npu.dev, irq,
			       mtk_npu_mbox_handler,
			       IRQF_ONESHOT,
			       pdev->name, NULL);
	if (ret) {
		NPU_NOTICE("request mailbox irq failed\n");
		return ret;
	}

	for (i = 0; i < CORE_MAX; i++) {
		INIT_LIST_HEAD(&mbox.core[MBOX_SEND][i].mdev_list);
		INIT_LIST_HEAD(&mbox.core[MBOX_RECV][i].mdev_list);
		spin_lock_init(&mbox.core[MBOX_SEND][i].lock);
		spin_lock_init(&mbox.core[MBOX_RECV][i].lock);
	}
