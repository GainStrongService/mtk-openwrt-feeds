...
static struct npu_runtime_monitor trm = {
					    ...
					    u32 *frag_len)
{
	*frag_len -= CORE_DUMP_FRAME_LEN;
	memcpy(dst + *frag_len, &cd_frames[trm_cfg->core], CORE_DUMP_FRAME_LEN);
}
			  ...
			  u32 dump_rsn)
{
	...
	while (total > 0) {
		...
		dst = mtk_npu_trm_fs_relay_reserve(frag_len + TRM_HDR_LEN);
		if (IS_ERR(dst))
			return PTR_ERR(dst);
...
int mtk_trm_cpu_utilization(enum core_id core, u32 *cpu_utilization)
{
	...
	*cpu_utilization = 0;
	...
	if (ret) {
		NPU_ERR("send CPU_UTILIZATION cmd failed(%d)\n", ret);
		return ret;
	}
...
int mtk_trm_dump(u32 rsn)
{
	...
	for (i = 0; i < __TRM_HARDWARE_MAX; i++) {
		...
		for (j = 0; j < trm_hw_cfg->cfg_len; j++) {
			...
			if (unlikely(trm_cfg_sanity_check(trm_cfg))) {
				NPU_ERR("trm %s: sanity check fail\n", trm_cfg->name);
				ret = -EINVAL;
				goto out;
			}
	...
	NPU_NOTICE("NPU runtime monitor dump\n");
...
EXPORT_SYMBOL(mtk_trm_dump);

static int mtk_npu_trm_register_mbox(void)
{
	int ret;
	int i;
	...
	if (ret) {
		NPU_ERR("register trm mgmt mbox send failed: %d\n", ret);
		return ret;
	}
	...
	for (i = 0; i < CORE_OFFLOAD_NUM; i++) {
		...
		if (ret) {
			NPU_ERR("register trm offload %d mbox send failed: %d\n",
				i, ret);
...
static void mtk_npu_trm_unregister_mbox(void)
{
	int i;
