...
#define AX_BCDDEVICE_ID_279A			0x0500
...
#define AX88179_MAX_TX					10
#define AX88179_MAX_RX					10 // 2 is for short packet
#define AX88772_MAX_TX					4
#define AX88772_MAX_RX					4
#ifdef ENABLE_LSO
#define AX88179_BUF_TX_SIZE				(1000 * KB_SIZE)
#else
#define AX88179_BUF_TX_SIZE				(81 * KB_SIZE)
#endif
#define AX88772_BUF_TX_SIZE				(24 * KB_SIZE)
#define TX_CASECADES_SIZE				(16 * KB_SIZE)
#define INTBUFSIZE						8
#define TX_ALIGN						4
#define RX_ALIGN						8
...
/*AX_TX_MAX_QUEUE_SIZE only determines the queue alloc to struct
  not the actual queue use*/
#define AX_TX_MAX_QUEUE_SIZE			4 

#define AX_TX_QUEUE_LEN					200
...
#ifdef ENABLE_LSO
#define IHL 							0x0F
#define NEXT_HDR 						0xFF
#define IPV4_URG_NON_LINEAR 			38
#define IPV4_URG_LINEAR 				52
#define IPV4_WIN_NON_LINEAR 			34
#define IPV4_WIN_LINEAR 				48
#define IPV6_URG_NON_LINEAR 			58
#define IPV6_URG_LINEAR 				72
#define IPV6_WIN_NON_LINEAR 			54
#define IPV6_WIN_LINEAR 				68
#endif
...
enum ax_chip_version {
	...
	AX_VERSION_AX88179A_772D	= 0x6,
	AX_VERSION_AX88279			= 0x7,
	AX_VERSION_AX88279A			= 0xF,
};
...
struct pkt_buff {
	int8_t 			index;
	u8 				len;
	struct frag_pkt *head;
	struct frag_pkt *prev;
	u32				total_len;
};

enum preempt_buff_action {
    AX_DISCARD = 1,
    AX_ADD,
    AX_DISCARD_N_ADD,
};
...
struct ax_device {
	...
	struct list_head 			rx_done, tx_free[AX_TX_MAX_QUEUE_SIZE];
	struct sk_buff_head 		tx_queue[AX_TX_MAX_QUEUE_SIZE];
	...
	u8 							fw_version[4];
	u32							ephy_fw_version;
	u16 						ep1_bytes_per_interval;
#ifdef ENABLE_INT_POLLING
    bool 						int_polling_enable;
#endif
...
struct driver_info {
	...
	int		(*hw_init)(struct ax_device *axdev, int no_pm);
...
(struct net_device *netdev, struct ethtool_pauseparam *pause);
void ax_check_rx_biq_size
(struct ax_device *axdev, struct _ax_buikin_setting *ax_bulk_set);
