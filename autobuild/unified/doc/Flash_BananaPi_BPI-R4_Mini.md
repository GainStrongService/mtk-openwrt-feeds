Flashing BananaPi BPI-R4 Mini
======================================

This document provides a step-by-step guide to flashing the BananaPi BPI-R4 Mini device using the MT7987 SoC with updated U-Boot and ATF configurations.
...
This section covers the bootloader upgrade process for BananaPi BPI-R4 Mini, including BL2/FIP image naming, obtaining U-Boot and ATF, and chainload flashing steps for both NMBM and UBI layouts.

### Introduction

BananaPi BPI-R4 Mini uses the MT7987 SoC. The bootloader consists of BL2, FIP, U-Boot, and ATF. The BL2 image supports both SPI0 and SPI2 in a single file and is shared with MT7987 RFB. The FIP image is specific to BPI-R4 Mini and must not use the RFB or BPI-R4-Lite image.
...
- Prebuilt chainload image(s) are provided by autobuild. Example filename:
  `autobuild_release/filogic/bootloader-mt7987/mt7987_bananapi_bpi-r4-mini-snand-nmbm-chainload-<timestamp>.img`
- GPT table image(s) for SD/eMMC are provided for convenience when switching layouts:
  `autobuild_release/filogic/GPT_SD_mt798x_itb-<timestamp>.bin`,
  `autobuild_release/filogic/GPT_EMMC_mt798x_itb-<timestamp>.bin`
...
- FIP: Must use BPI-R4 Mini image.
  - EMMC: `mt7987_bananapi_bpi-r4-mini-emmc-u-boot-<date>.fip`
  - SDMMC: `mt7987_bananapi_bpi-r4-mini-sdmmc-u-boot-<date>.fip`
  - SNAND/NMBM: `mt7987_bananapi_bpi-r4-mini-snand-nmbm-u-boot-<date>.fip`
  - SNAND/UBI: `mt7987_bananapi_bpi-r4-mini-snand-u-boot-<date>.fip`
...
1. **Use prebuilt chainload image generated by autobuild** (no manual mkimage required).
2. **Load and chainload U-Boot** via TFTP.
3. **Upgrade BL2 and FIP**:
   - Use `mtkupgrade` in U-Boot.
   - BL2: `autobuild_release/filogic*/bootloader-mt7987/mt7987-spim-nand0-nmbm-comb-bl2-*.img`
   - FIP: `autobuild_release/filogic*/bootloader-mt7987/mt7987_bananapi_bpi-r4-mini-snand-nmbm-u-boot-*.fip`
...
3. **Upgrade BL2 and FIP**:
   - Use `mtkupgrade` in U-Boot.
   - BL2: `autobuild_release/filogic*/bootloader-mt7987/mt7987-spim-nand0-ubi-comb-bl2-*.img`
   - FIP: `autobuild_release/filogic*/bootloader-mt7987/mt7987_bananapi_bpi-r4-mini-snand-u-boot-*.fip`

---

## Important Notes on BL2 and FIP Image Naming for BananaPi BPI-R4 Mini

### BL2 Image Naming

- Although BananaPi BPI-R4 Mini uses SPI2 instead of SPI0, the BL2 image supports both in a single file.
...
- BananaPi BPI-R4 Mini must use its own FIP images, not the RFB or BPI-R4-Lite images.
- Example FIP image names (ignore the date portion):
  - For EMMC: `mt7987_bananapi_bpi-r4-mini-emmc-u-boot-<date>.fip`
  - For SDMMC: `mt7987_bananapi_bpi-r4-mini-sdmmc-u-boot-<date>.fip`
  - For SNAND/NMBM: `mt7987_bananapi_bpi-r4-mini-snand-nmbm-u-boot-<date>.fip`
  - For SNAND/UBI: `mt7987_bananapi_bpi-r4-mini-snand-u-boot-<date>.fip`
  - (All located in `autobuild_release/filogic/bootloader-mt7987/`)
...
1. **Use the prebuilt chainload image generated by autobuild**
   - Example: `autobuild_release/filogic/bootloader-mt7987/mt7987_bananapi_bpi-r4-mini-snand-nmbm-chainload-<timestamp>.img`
2. **Load the chainload image into DRAM via TFTP**
   ```
   setenv serverip <your_tftp_server_ip>
   tftpboot 0x44000000 mt7987_bananapi_bpi-r4-mini-snand-nmbm-chainload-<timestamp>.img
   ```
...
4. **Upgrade BL2 and FIP using the MediaTek official U-Boot**
   ...
   - Use the boot menu or U-Boot commands to flash BL2 and FIP from the `autobuild_release` directory:
     ...
     - `ATF BL2`: `autobuild_release/filogic*/bootloader-mt7987/mt7987-spim-nand0-nmbm-comb-bl2-*.img`
       > Note: This BL2 image is shared with MT7987 RFB. Use the latest available image; the date may differ.
     - `ATF FIP`: `autobuild_release/filogic*/bootloader-mt7987/mt7987_bananapi_bpi-r4-mini-snand-nmbm-u-boot-*.fip`
       > Note: This FIP image is specific to BPI-R4 Mini. Do not use the RFB or BPI-R4-Lite FIP image.
...
Input file name: openwrt-mediatek-filogic-bananapi_bpi-r4-mini-squashfs-sysupgrade.itb
```
...
```
setenv bootconf mt7987a-bananapi-bpi-r4-mini-nand-nmbm
saveenv
```
...
#### MT7987 Board-Specific Overlays for BananaPi BPI-R4 Mini (25.12 SDK)

Select the overlay according to your desired boot medium:

- Boot from NAND Flash (NMBM): `mt7987a-bananapi-bpi-r4-mini-nand-nmbm`
- Boot from NAND Flash (Full-UBI): `mt7987a-bananapi-bpi-r4-mini-nand`
- Boot from eMMC: `mt7987a-bananapi-bpi-r4-mini-emmc`
- Boot from SD Card: `mt7987a-bananapi-bpi-r4-mini-sd`

Configuration can be carried out via the bootmenu or by directly setting environment variables. Example configuration:

```bash
setenv bootconf mt7987a-bananapi-bpi-r4-mini-nand
saveenv
```

By following these procedures, you can ensure your BananaPi BPI-R4 Mini is properly configured and flashed for optimal performance.
...
1. **Use the prebuilt chainload image generated by autobuild**
   - Example: `autobuild_release/filogic/bootloader-mt7987/mt7987_bananapi_bpi-r4-mini-snand-chainload-<timestamp>.img`
2. **Load the chainload image into DRAM via TFTP**
   ```
   setenv serverip <your_tftp_server_ip>
   tftpboot 0x44000000 mt7987_bananapi_bpi-r4-mini-snand-chainload-<timestamp>.img
   ```
3. **Chainload the MediaTek official U-Boot**
   ```
   setenv autostart 1
...
4. **Upgrade BL2 and FIP using the MediaTek official U-Boot**
   ...
   - Use the boot menu or U-Boot commands to flash BL2 and FIP from the `autobuild_release` directory:
     - Command: Use the `mtkupgrade` command to upgrade the relevant images.
     - `ATF BL2`: `autobuild_release/filogic*/bootloader-mt7987/mt7987-spim-nand0-ubi-comb-bl2-*.img`
       > Note: This BL2 image is shared with MT7987 RFB. Use the latest available image; the date may differ.
     - `ATF FIP`: `autobuild_release/filogic*/bootloader-mt7987/mt7987_bananapi_bpi-r4-mini-snand-u-boot-*.fip`
       > Note: This FIP image is specific to BPI-R4 Mini. Do not use the RFB or BPI-R4-Lite FIP image.
5. **Flash OpenWrt Firmware:**
   ```
   setenv bootconf mt7987a-bananapi-bpi-r4-mini-nand
   ...
   - Use the boot menu or U-Boot commands to flash OpenWrt from the `autobuild_release` directory:
     - Command: Use the `mtkupgrade` command to upgrade the relevant images.
     - `Firmware`: `bin/targets/mediatek/*/openwrt-mediatek-filogic-bananapi_bpi-r4-mini-squashfs-sysupgrade.itb`
       (Or `./autobuild_release/*/openwrt-mediatek-filogic-bananapi_bpi-r4-mini-squashfs-sysupgrade-*.itb`)
   - After the firmware upgrade, U-Boot will automatically boot into OpenWrt.
...
*   Use the image named `openwrt-mediatek-filogic-bananapi_bpi-r4-mini-squashfs-sysupgrade.itb` regardless of whether the board uses SPIM-NAND or EMMC to perform the firmware upgrade.
...
Configuration can be carried out via the bootmenu or directly setting environment variables. Example configuration:
...
|:---      |:---        |:---       |:---                                                                                                   |
| v1.0     | 2026/03/11 | Sam Shih  | Initial BPI-R4-Mini flash doc based on BPI-R4-Lite; updated FIP names, overlay names, chainload/GPT paths |
